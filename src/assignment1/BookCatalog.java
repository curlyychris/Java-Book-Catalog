...
public class BookCatalog {
	...
	private void placeFileInBook(String lineFromFile)
	{	
		for(int i=0;i<books.length;i++)
		{
			 //if a position is empty it will place the contents of the file 
		    //into that position
			if(books[i]==null) 
			{
				Book book=new Book();
				//splittingLine[0] is now:
				// Algorithms to Live By: The Computer Science of Human Decisions?
				//and splittingLine[1] is now:
				//Brian Christian 1
				//breaks the line based on the delimiter - and it doesnt include it
				String[]splittingLine=lineFromFile.split("-");
				book.setBookTitle(splittingLine[0]);
				//now im splitting the Brian Christian 1 part 
				splittingLine=splittingLine[1].split(" ");
				book.setAuthorFirstName(splittingLine[0]);
				book.setAuthorsLastName(splittingLine[1]);
				//its a string array so you have to parse the edition number
				//since its an integer
				book.setEditionNumber(Integer.parseInt(splittingLine[2]));
				books[i]=book;
				break;
			}
		}	
